...
import {useHoldingContract} from "@/hooks/useHoldingContract";
...
export default function Step0() {
    ...
    const { tco2, solanaAccountAddress, owner, error, isLoading } = useHoldingContract();
    ...
    useEffect(() => {
        if (tco2) getProjectById(tco2).then(setRetirementDetails);
    }, [tco2]);
    ...
    return (<div>
        ...
        {isLoading && <div className="text-gray-500 mb-2">Loading...</div>}
        {error && <div className="text-red-500 mb-2">Error: {error.message}</div>}
        Retirement Project: {tco2}
        SolanaAccount: {solanaAccountAddress}
        Owner: {owner}
        Details: {JSON.stringify(retirementDetails, null, 2)}
        <ToucanProjectsSelector />
